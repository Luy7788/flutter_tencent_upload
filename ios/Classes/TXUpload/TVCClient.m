    ...
    if ([manager fileExistsAtPath:param.videoPath]) {
        ...
        if (uploadContext.isUploadCover) {
            if ([manager fileExistsAtPath:param.coverPath]) {
                ...
                uploadContext.coverLastModTime = [[[manager attributesOfItemAtPath:param.coverPath error:nil] fileModificationDate] timeIntervalSince1970];
            } else {
            ...
            if (uploadContext.resumeData != nil && uploadContext.resumeData.length != 0) {
                ...
                [videoUpload setInitMultipleUploadFinishBlock:^(QCloudInitiateMultipartUploadResult *multipleUploadInitResult, QCloudCOSXMLUploadObjectResumeData resumeData) {
                    if (multipleUploadInitResult != nil && resumeData != nil) {
                        [self setSession:cug.uploadSession resumeData:resumeData lastModTime:uploadContext.videoLastModTime coverLastModTime:uploadContext.coverLastModTime withFilePath:param.videoPath];
                    }
                }];
            }
...
- (NSString *)getSessionFromFilepath:(TVCUploadContext *)uploadContext {
    ...
    coverLastModTimeDic = [NSJSONSerialization JSONObjectWithData:[strPathToCoverLastModTime dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingAllowFragments error:&jsonErr];
    if (jsonErr) {
...
- (void)setSession:(NSString *)session resumeData:(NSData *)resumeData lastModTime:(uint64_t)lastModTime coverLastModTime:(uint64_t)coverLastModTime withFilePath:(NSString *)filePath {
    if (filePath == nil || filePath.length == 0) {
        return;
    }
    ...
    if (strPathToCoverLastModTime) {
        NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:[strPathToCoverLastModTime dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingAllowFragments error:&jsonErr];
