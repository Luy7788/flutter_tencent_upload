...
- (NSMutableDictionary*)getCacheDicByKey:(NSString*)key {
    ...
    if (str == nil) {
        NSLog(@"%@ is nil", TVCMultipartResumeSessionKey);
        return nil;
    }
    NSMutableDictionary *dic = [NSJSONSerialization JSONObjectWithData:[str dataUsingEncoding:NSUTF8StringEncoding]
                                                               options:NSJSONReadingAllowFragments
                                                                 error:&jsonErr];
    if (jsonErr) {
        NSLog(@"%@ is not json format: %@", TVCMultipartResumeSessionKey, str);
        return nil;
    }
    return dic;
}
